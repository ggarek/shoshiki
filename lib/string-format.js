...
//                '; intFormat='+integerFormat+'; fFormat='+fractionalFormat+'; sepIdx='+separatorIdx);
  ...
  function formatIntegerPart(number, mask) {
    ...
    for(i=len-1, j=str.length-1; i>=0; i--, j--){
      ...
      switch(mask[i]){
        case '0':
          // 'Flush' buffer with custom characters
          if(buff.length > 0){
            result = result.concat(buff);
            buff.length = 0;
          }
          // Fill pending empty symbols with '0', because we hit '0' in format token
          if(emptyCount > 0){
        ...
        case '#':
          if(digit === null){
            // Increase pending empty symbols count (they should be added to result only if we hit '0' in format token)
            emptyCount++;
          } else {
            // 'Flush' buffer with custom characters
            if(buff.length > 0){
...
//      console.log('d='+digit+'; mc='+maskChar+';res='+result);
    ...
    // Push digits if some left
    for(i=j; j>=0; j--){
      result.push(str.charAt(j));
    }
    
    // Push custom characters
    if(buff.length > 0){
      result = result.concat(buff);
      buff.length = 0;
    }
